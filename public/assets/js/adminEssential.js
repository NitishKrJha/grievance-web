...
{
	if($("#elements").val()==''){
		alert('Please select atlist one item.');
	}else{
		$("#deleteForm").submit();	
	}
}

function Active_all(action_url)
{
	var val = '';
	jQuery("INPUT[name='check_box'][type='checkbox']:checkbox:checked").each(function(i){
	...
	if(val==''){
		...
		//jq_alert("Please select atleast one Item!");
		return false;
	}else{
		//if(jq_confirm("Are you sure to change the status of the selected items to active")){
		if(confirm("Are you sure to change the status of the selected items to active")){
			document.fromCheckAction.check_ids.value = val;
			document.fromCheckAction.action = hostname+action_url;
			document.fromCheckAction.submit();
		}
	}
}

function Inactive_all(action_url)
{
	var val = '';
	jQuery("INPUT[name='check_box'][type='checkbox']:checkbox:checked").each(function(i){
	  if(jQuery(this).val()!='Yes')
	...
	if(val==''){
		...
		//if(jq_confirm("Are you sure to change the status of the selected items to inactive")){
		if(confirm("Are you sure to change the status of the selected items to inactive")){
			document.fromCheckAction.check_ids.value = val;
			document.fromCheckAction.action = hostname+action_url;
			document.fromCheckAction.submit();
		}
...
function Trash_all(action_url)
{
	...
	if(val==''){
		...
		//if(jq_confirm("Are you sure to change the status of the selected items to inactive")){
		if(confirm("Are you sure to send of the selected items to Trash")){
			document.fromCheckAction.check_ids.value = val;
			document.fromCheckAction.action = hostname+action_url;
			document.fromCheckAction.submit();
		}
...
function Restore_all(action_url)
{
	var val = '';
	jQuery("INPUT[name='check_box'][type='checkbox']:checkbox:checked").each(function(i){
	  if(jQuery(this).val()!='Yes')
	...
	if(val==''){
		...
		//if(jq_confirm("Are you sure to change the status of the selected items to inactive")){
		if(confirm("Are you sure to restore the selected items")){
			document.fromCheckAction.check_ids.value = val;
			document.fromCheckAction.action = hostname+action_url;
			document.fromCheckAction.submit();
		}
