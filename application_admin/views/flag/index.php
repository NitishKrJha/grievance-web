        ...
        <table class="table table-striped jambo_table bulk_action">
          <thead>
            <tr class="headings">
              ...
              <th class="column-title">From Member Name</th>
              <th class="column-title">To Menber Name</th>
          ...
          <?php if(!empty($recordset)){ $i=2; ?>
            <?php $i=1; foreach($recordset as $singleRecord){
              $ediLink = str_replace('{{ID}}', $singleRecord['flag_id'], $edit_link);
              $activeLink = str_replace('{{ID}}',$singleRecord['flag_id'],$activated_link);
			        $inacttivedLink = str_replace('{{ID}}',$singleRecord['flag_id'],$inacttived_Link);
            ?>
            <tr class="<?php echo $i%2==0?'even':'odd'; ?> pointer">
			        <td class=" "><?php echo $i+$startRecord; ?></td>
			        <td class=" "><?php echo $singleRecord['fromName']; ?></td>
              <td class=" "><?php echo $singleRecord['toName']; ?></td>
              <td class=" "><?php echo $singleRecord['message']; ?></td>
               <td class=" "><?php echo date('Y m d H:i:s',strtotime($singleRecord['created_date'])); ?></td>
             
             <td><a href="<?php echo base_url('member/viewdetails/'.$singleRecord['member_id']); ?>" target="_blank">View Details</a></td>
...
<script>
function ChangeStatusForBooking(val,flag_id){
  if(val==0){
        ...
        <div class="modal-content">
            <form name="sendForm" id="sendForm" action="<?php echo base_url('booking/givepermission'); ?>" method="post">
              ...
              <div class="modal-body mx-3">
                  ...
                  <input type="hidden" name="flag_id" id="flag_id" value="0">
