    ...
    <div class="x_panel">
      ...
      <ul class="parsley-errors-list filled error text-left" ><li class="parsley-required"><?php echo $errmsg; ?></li></ul>
		<form id="form1" class="form-horizontal" action="<?php echo $do_addedit_link;?>" method="post" enctype="multipart/form-data">
			<span class="section">Contact Type</span>
				<div class="form-group">
					<label class="control-label col-md-3 col-sm-3 col-xs-12">Contact Industry <span class="required">*</span></label>
					<div class="col-md-6 col-sm-6 col-xs-12">
						<select class="form-control validate[required]" name="contact_industry" id="contact_industry">
							<option value="">Select Industry</option>
							<?php foreach($contactIndustrys as $contactIndustry){?>
								<option value="<?php echo $contactIndustry['id']; ?>" <?php if($contact_industry_id==$contactIndustry['id']){ ?>selected<?php } ?>><?php echo $contactIndustry['title'];?></option>
							<?php } ?>
						</select>
					</div>
				</div>
