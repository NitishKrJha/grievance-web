<?php
class Premiumadsprice extends CI_Controller {
	function __construct()
	{
		parent::__construct();
		$this->load->model('ModelPremiumadsprice');
		$this->controller = 'premiumadsprice';
	}
	...
	function index()
	{
		...
		$data['recordset'] 		= $this->ModelPremiumadsprice->getList($config,$start,$param);
		...
		$elements['main'] = 'premiumadsprice/index';
	...
	function addedit($id = 0)
	{
		...
		if($contentId > 0)
		{
			...
			$data['action'] = "Edit";
			//=================prepare DATA ==================
			$rs = $this->ModelPremiumadsprice->getSingle($contentId);
			//$row = $rs->fields;
			if(is_array($rs[0]))
			{
		...
		$elements['menu'] = 'menu/index';
		$elements['topmenu'] = 'menu/topmenu';
		$elements['main'] = 'premiumadsprice/add_edit';
		$element_data['main'] = $data;
		$element_data['main'] = $data;
	...
	function do_addedit()
	{
		...
		if($this->form_validation->run() == TRUE)
		{
			if($contentId > 0)   //edit
			{
				...
				$affected_row = $this->ModelPremiumadsprice->editContent($data,$contentId);
				$this->nsession->set_userdata('succmsg', 'Successfully Premium Ads Price Edited.');
