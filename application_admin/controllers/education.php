...
class Education extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('ModelEducation');
	}
	...
	function index()
	{
		...
		$data['recordset'] 		= $this->ModelEducation->getList($config,$start,$param);
		$data['startRecord'] 	= $start;
		$data['module']			= "Education Management";
		$this->pagination->initialize($config);

		$data['params'] 			= $this->nsession->userdata('ADMIN_EDUCATION');
	...
	function addedit($id = 0)
	{
		...
		if($contentId > 0)
		{
			...
			$data['action'] = "Edit";
			//=================prepare DATA ==================
			$rs = $this->ModelEducation->getSingle($contentId);
			//pr($rs);
			//$row = $rs->fields;
			if(is_array($rs))
			{
		...
		$elements['menu'] = 'menu/index';
		$elements['topmenu'] = 'menu/topmenu';
		$elements['main'] = 'education/add_edit';
	...
	if($contentId==0){
		$this->ModelEducation->addContent($data);
		$this->nsession->set_userdata('succmsg','Education added successfully.');
		redirect(base_url($this->controller."/index/"));
	}else{
		$this->ModelEducation->editContent($data,$contentId);
		$this->nsession->set_userdata('succmsg','Education updated successfully.');
		redirect(base_url($this->controller."/index/"));
	}
	}
	...
	function activate()
	{
		...
		$this->ModelEducation->activate($id);
		// $result = $this->ModelEducation->getsingle_empdata($id);
		// $email = $result->email;
		...
		$this->nsession->set_userdata('succmsg', 'Successfully Education Activated.');
		redirect(base_url($this->controller."/index/"));
		return true;
	}
	function inactive()
	{
		$this->functions->checkAdmin($this->controller.'/');
		$id = $this->uri->segment(3, 0);
		$this->ModelEducation->inactive($id);
		// $result 		= $this->ModelEducation->getsingle_empdata($id);
		// $email 			= $result->email;
		// $first_name = $result->first_name;
		...
		// $return_check = $this->functions->mail_template($to,$subject,$body);
		$this->nsession->set_userdata('succmsg', 'Successfully Education Inactivated.');
		redirect(base_url($this->controller."/index/"));
		return true;
	}
	function delete($id){
		$checkIfApply = $this->ModelEducation->checkIfApply($id);
		if($checkIfApply>0){
		$this->nsession->set_userdata('errmsg', 'This Education type is already applied.');
		redirect(base_url($this->controller."/index/"));
		}else{
		$this->functions->checkAdmin($this->controller.'/');
		$this->ModelEducation->delete($id);
		$this->nsession->set_userdata('succmsg', 'Successfully Education deleted.');
		redirect(base_url($this->controller."/index/"));
		}
	}
}
