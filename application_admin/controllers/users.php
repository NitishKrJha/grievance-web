...
class Users extends CI_Controller {
	...
	function index()
	{
		...
		$data['recordset'] 		= $this->ModelUsers->getList($config,$start,$param);
		$data['startRecord'] 	= $start;
		$data['module']			= "users";
		$this->pagination->initialize($config);

		$data['params'] 			= $this->nsession->userdata('ADMIN_users');
	...
	function addedit($id = 0)
	{
		...
		$data['allcountry']		= $this->ModelUsers->getCountry();
		$data['id']=$id;
		if($contentId > 0)
		{
			$data['id']	= $id;
			...
			$data['action'] = "Edit";
			//=================prepare DATA ==================
			$rs = $this->ModelUsers->getSingle($contentId);
	...
	function do_addedit()
	{
		...
		if($_FILES['picture']['name'])
        {
			...
			if(!$this->upload->do_upload('picture')) {
				...
				redirect(base_url('users/addedit/'.$contentId));
				return true;
			} else {
		...
		if($contentId==0){
			...
			$result = $this->ModelUsers->addContent($data,$Imgdata);
			
			$to 				= $data['email'];
			$subject			='MMR users Registration';
			$body='<tr><td>Hi,</td></tr>
					<tr style="background:#fff;"><td>We have added you as a users . </td></tr>
					<tr><td>So, Please complete next step to enter our site</td></tr>
					<tr><td><a href="'.front_base_url().'page/users/'.base64_encode($to).'">Click Here</a></td></tr>';
			$this->functions->mail_template($to,$subject,$body);
			$this->nsession->set_userdata('succmsg','users added successfully.');
			...
			$this->ModelUsers->editContent($data,$contentId,$Imgdata);
			
			
			if(count($_FILES["certificate"]["name"])>0)
			{
				$config["upload_path"] = file_upload_absolute_path().'users_certificate/';
				$config["allowed_types"] = 'gif|jpg|png';
				$this->upload->initialize($config);
				for($count = 0; $count<count($_FILES["certificate"]["name"]); $count++)
				{
					...
					if($this->upload->do_upload('certificates'))
					{
						...
						$certificateData[$count]['certificate'] = file_upload_base_url().'users_certificate/'.$Imgdata1["file_name"];
						$certificateData[$count]['member_id'] = $contentId;
						//$this->ModelUsers->addcertificate($certificateData);
					}
				}
			...
			$this->nsession->set_userdata('succmsg','users updated successfully.');
			redirect(base_url($this->controller));
		}
	}
	...
	function activate()
	{
		...
		$this->ModelUsers->activate($id);
		 $result = $this->ModelUsers->getsingle_empdata($id);
		 $email 		= $result->email;
		...
		$this->functions->mail_template($to,$subject,$body);
		$this->nsession->set_userdata('succmsg', 'Successfully users Activated.');
	...
	function inactive()
	{
		...
		$this->functions->mail_template($to,$subject,$body);
		$this->nsession->set_userdata('succmsg', 'Successfully users Inactivated.');
		redirect(base_url($this->controller."/index/"));
		return true;
	}
	function viewdetails($id){
		if($id){
			$rs = $this->ModelUsers->getSingle($id);
			//pr($rs);
			//$row = $rs->fields;
			if(is_array($rs))
			{
			...
			$elements['menu'] = 'menu/index';
			$elements['topmenu'] = 'menu/topmenu';
			$elements['main'] = 'users/view_details';
			$element_data['menu'] = $data;//array();
			$element_data['main'] = $data;
			$this->layout->setLayout('layout_main_view');
			$this->layout->multiple_view($elements,$element_data);
	...
	public function emailExist($id=0){
		...
		$return = $this->ModelUsers->checkEmail($email_id,$id);
		if(count($return)!=''){
			$error = "false";
		}else{
			$error = "true";
	...
	public function phoneExist($id=0){
		...
		$return = $this->ModelUsers->checkPhone($phone,$id);
		if(count($return)!=''){
			$error = "false";
		}else{
			$error = "true";
	...
	public function crnExist($id=0){
		...
		$return = $this->ModelUsers->checkCrn($crn,$id);
		if(count($return)!=''){
			$error = "false";
		}else{
			$error = "true";
	...
	public function do_delete(){
		$this->functions->checkAdmin($this->controller.'/');
		$id = $this->uri->segment(3, 0);
		$this->ModelUsers->doDeleteMemeber($id);
		$result 		= $this->ModelUsers->getsingle_empdata($id);
		$email 			= $result->email;
	...
	function delete($id){
		...
		$this->nsession->set_userdata('succmsg', 'Successfully users deleted.');
		redirect(base_url($this->controller."/index/"));
	}
	...
	function delete_certificate($id,$member_id){
		...
		$this->functions->checkAdmin($this->controller.'/');
		$this->ModelUsers->delete_certificate($id);
		$this->nsession->set_userdata('succmsg', 'Successfully users deleted.');
