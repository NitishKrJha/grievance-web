...
class Pet extends CI_Controller {
	...
	function index()
	{
		...
		$data['recordset'] 		= $this->ModelPet->getList($config,$start,$param);
		$data['startRecord'] 	= $start;
		$data['module']			= "Pet Management";
		$this->pagination->initialize($config);

		$data['params'] 			= $this->nsession->userdata('ADMIN_PET');
	...
	function addedit($id = 0)
	{
		...
		if($contentId > 0)
		{
			...
			$data['action'] = "Edit";
			//=================prepare DATA ==================
			$rs = $this->ModelPet->getSingle($contentId);
			//pr($rs);
			//$row = $rs->fields;
			if(is_array($rs))
			{
	...
	if($contentId==0){
		$this->ModelPet->addContent($data);
		$this->nsession->set_userdata('succmsg','Pet added successfully.');
		redirect(base_url($this->controller."/index/"));
	}else{
		$this->ModelPet->editContent($data,$contentId);
		$this->nsession->set_userdata('succmsg','Pet updated successfully.');
		redirect(base_url($this->controller."/index/"));
	}
	}
	...
	function activate()
	{
		...
		$this->ModelPet->activate($id);
		// $result = $this->ModelPet->getsingle_empdata($id);
		// $email = $result->email;
		...
		$this->nsession->set_userdata('succmsg', 'Successfully Pet Activated.');
		redirect(base_url($this->controller."/index/"));
		return true;
	}
	function inactive()
	{
		...
		// $return_check = $this->functions->mail_template($to,$subject,$body);
		$this->nsession->set_userdata('succmsg', 'Successfully Pet Inactivated.');
		redirect(base_url($this->controller."/index/"));
		return true;
	}
	function delete($id){
		$checkIfApply = $this->ModelPet->checkIfApply($id);
		if($checkIfApply>0){
		$this->nsession->set_userdata('errmsg', 'This Pet Name is already applied.');
		redirect(base_url($this->controller."/index/"));
		}else{
		$this->functions->checkAdmin($this->controller.'/');
		$this->ModelPet->delete($id);
		$this->nsession->set_userdata('succmsg', 'Successfully Pet deleted.');
		redirect(base_url($this->controller."/index/"));
		}
	}
}
