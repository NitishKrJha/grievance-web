...
class Flag extends CI_Controller {
	...
	function index()
	{
		...
		$param['sortField'] 		= $this->input->request('sortField','flag_id');
		...
		$data['recordset'] 		= $this->ModelFlag->getList($config,$start,$param);
		
		//pr($data['recordset']);
		
		$data['startRecord'] 	= $start;
		
		$data['module']			= "Counselor flag details";
	...
	function activate()
	{
		...
		$this->ModelFlag->activate($id);
	
		$result 		= $this->ModelFlag->getsingle_empdata($id);
		
				$to 			= $result[0]['member_email'];
				$subject		= "flag Confirm";
	...
	function givepermission(){
		$flag_id=($this->input->post('flag_id'))?$this->input->post('flag_id'):0;
		$flag_status=($this->input->post('flag_status'))?$this->input->post('flag_status'):0;
		if($flag_id > 0){
			$n_c_data=array();
			$n_m_data=array();
			if($flag_status==0){
				
				$this->ModelFlag->inactive($flag_id);
				$result 		= $this->ModelFlag->getsingle_empdata($flag_id);
				$n_c_data['member_id']=$result[0]['member_id'];
				$n_c_data['site_url']='member/flag_notification';
				...
				$to 			= $result[0]['member_email'];
				$subject		= ($this->input->post('subject'))?$this->input->post('subject'):"flag Confirm";
				$body			= "<tr><td>Hi, Member</td></tr>
									<tr><td> Admin Assign to ".$result[0]['counselorName']." </td></tr>";
				...
				$this->functions->mail_template($to,$subject,$body);
				$to1 			= $result[0]['counselorEmail'];
				$subject1		= ($this->input->post('c_subject'))?$this->input->post('c_subject'):"flag Assign";
				$body1			= "<tr><td> Hi,Counselor </td></tr>
				...
				$this->ModelFlag->insertData('member_notification',$n_m_data);
				$this->ModelFlag->insertData('member_notification',$n_c_data);
				$this->nsession->set_userdata('succmsg', 'Successfully Assign.');
				echo 'true';
			}else{
				$this->ModelFlag->activate($flag_id);
				$result 		= $this->ModelFlag->getsingle_empdata($flag_id);
				...
				$to 			= $result[0]['member_email'];
				$subject		= ($this->input->post('subject'))?$this->input->post('subject'):"flag Canceled";
				$body			= "<tr><td>Hi, Member</td></tr>
									<tr><td> Admin Assign to ".$result[0]['counselorName']." </td></tr>";
				if($this->input->post('message')){
					$body			= $this->input->post('message');
				...
				$to1 			= $result[0]['counselorEmail'];
				$subject1		= ($this->input->post('c_subject'))?$this->input->post('c_subject'):"flag Canceled";
				$body1			= "<tr><td> Hi,Counselor </td></tr>
									<tr><td> ".$result[0]['memberName']." has been assigned to you.</td></tr>";
				if($this->input->post('c_message')){
					$body1			= $this->input->post('c_message');
