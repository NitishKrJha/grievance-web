		  ...
		  <div class="row">
		   ...
		   <!-- search section -->
			...
			<div class="row searchCounselor">
						...
						<?php
							$searchUrl=base_url().'member/mymatch';
							if($type=='paid'){
								$searchUrl=base_url().'member/mymatch/'.$type;
							}
						?>		
						<form action ="<?php echo $searchUrl;?>" method="post" name="search_counselor">
		   ...
		   $membership_expire_date = isset($mbr['membership_plan']['expiry_date'])?$mbr['membership_plan']['expiry_date']:'';		   
		   ?>
			...
			<a href="javascript:void(0);" identifier="<?php echo $mbr['member_id'];?>" onclick="javascript:chatWithData()" class="chatShowHidePre"><input type="hidden" value="<?php echo $mbr['name'];?>" class="h_name<?php echo $mbr['member_id'];?>" id="h_name<?php echo $mbr['member_id'];?>" /><input type="hidden" value="<?php echo $pic;?>" id="h_pic<?php echo $mbr['member_id'];?>" /><i class="fa fa-comment-o" aria-hidden="true"></i> </a>
		...
		$(window).scroll(function(){
			...
			if(total_record1 > parseInt(0)){
				$.ajax({
					...
					data : {'search_string':search_string,'type':'<?php echo $type; ?>'},
					dataType : 'json',
					type : 'post',
					beforeSend : function(){
					  // $('#loading').show();
					...
					success : function(data){
						...
						if(data.result.length > 0)
						{
							for(i=0;i<data.result.length;i++)
							{
								...
								var membership_expire_date ='';
								if(data.result[i].membership_plan_cnt > 0){
									var membership_expire_date =data.result[i].membership_plan['expiry_date'];
								}
								var exp_date="<?php echo $exp_date; ?>";
								var text = '';
								if( name !='' && age > 0  )
								{
									var text = name +','+age ;
								...
								else if(name !='' && age == 0 )
								{
									...
									var ddt="<?php echo date('Y-m-d'); ?>";
									html +='<div class="col-md-4 col-sm-4 matchMember">';
									html +='<div class="myMatch">';
									html +='<figure><img src="'+image+'" alt=""></figure>';
									html +='<div class="textSec">';
									if((membership_expire_date!='' && membership_expire_date >= ddt) && (exp_date!='' && exp_date>=ddt)){
										html +='<a href="javascript:void(0);" class="openVideoBox" identifier="'+member_id+'"><i class="fa fa-video-camera" aria-hidden="true"></i> </a>';
									}
									html +='<a href="javascript:void(0);" identifier="'+member_id+'" onclick="javascript:chatWithData()" class="chatShowHidePre"><input type="hidden" value="'+name+'" class="h_name'+member_id+'" id="h_name'+member_id+'" /><input type="hidden" value="'+image+'" id="h_pic'+member_id+'" /><i class="fa fa-comment-o" aria-hidden="true"></i> </a>';
									html +='<p><a href="<?= base_url('member/profile/')?>'+btoa(data.result[i].id)+'" class="chatName">'+text+'</a></p>';
									html +='</div>';
									html +='</div>';
									html +='</div>';
							}
...
<script>
 ...
 function messagealert123(title,text,type){
      new PNotify({
            title: title,
            text:  text,
            type:  type,
 ...
 function chatWithData(){
 	messagealert123('Error','This user did not activated for chat yet','error');
 }
