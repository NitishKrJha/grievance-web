		...
		<div class="col-md-12 myMatchSection">
		  <div class="row">
		   ...
		   <!-- search section -->
			...
			<div class="row searchCounselor">
                    	<div class="col-md-12 col-sm-12"><h3> Block Member List </h3></div>
		   ...
		   <!-- search section-->
		   <div id="search_div">
		   <?php if(!empty($blockData)){ foreach($blockData as $mbr){
		  ...
		  $membership_expire_date = isset($mbr['membership_plan'][0]['expiry_date'])?$mbr['membership_plan'][0]['expiry_date']:'';
		   ?>
			...
			<a href="javascript:void(0);" class="makeCanceled" data-idto="<?php echo $mbr['to_member_id'];?>">
							...
							for(i=0;i<data.result.length;i++)
							{
								...
								else if(name !='' && age == 0 )
								{
									...
									if((expiry_date!='' && expiry_date >= '<?= date(Y-m-d)?>') && (exp_date!='' && exp_date >= '<?= date('Y-m-d')?>')){
									 ...
									 html = html+'<p><a href="<?= base_url('member/profile/')?>'+btoa(data.result[i].id)+'">'+text+'</a></p><a href="javascript:void(0);" class="makeCanceled" data-id="'+data.result[i].id+'"><i class="fa fa-ban" aria-hidden="true" ></i></a></div></div></div>';
...
<script>
 $(document).on('click','.makeCanceled',function(){
	...
	var url='<?php echo base_url(); ?>member/unblockmember/'+id;
	
	if(window.confirm("Are you sure to Unblock this member ?"))
	{
