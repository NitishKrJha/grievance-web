...
class Functions
{
	...
	function checkPaidOrNot($id){
		$data=$this->obj->db->get_where('member_buyed_plan',array('member_id'=>$id,'is_active'=>1))->result_array();
		//echo "<pre>"; print_r($data);
		$expiry_date=isset($data[0]['expiry_date'])?$data[0]['expiry_date']:'';
	...
	function checkUser($section_name)
	{
		...
		if(!$UserID)
		{
			...
			$recordSet = $this->obj->db->select('id')->get_where('member',array('id'=>$UserID,'is_active'=>1))->num_rows();
			if($recordSet > 0){
				return true;
			}else{
	...
	function checkAdmin($section_name,$allowed=false)
	{
		...
		if(!$UserID)
		{
			...
			if($userType==1){ //super admin
				$access = array('All');
			}elseif($userType==2){ //user team access
				$access = array();
			}elseif($userType==3){ //quality
				$access = array();           	
			}else{ //super admin
				$access = array('ftpclient/', 'inventory/', 'campaigns/', 'teammembers/');
			}
   ...
   // --------------------------------------------------------------------
	...
	function mail_template_old($to,$subject,$body){
		...
		$mailbody = '<table width="100%" border="0" cellspacing="10" cellpadding="10" bordercolor="#000000" style="border-collapse:collapse; margin:0 auto; border:#C4C4C4 1px solid;">
					...
					<tr style="padding-top:20px;">
						<td style="background:#C00000;"><p style="color: #ffffff; font-family: Arial,Helvetica,sans-serif; font-size: 11px; padding: 6px 0;text-align: center;"> '.$global_site_name.' &copy; Copyright '.$serverYear.'. All Rights Reserved.</p></td>
					</tr>
				</table>';
        ...
        $this->obj->email->from($global_email,'MMR');
        $this->obj->email->subject($subject);
        $this->obj->email->message($mailbody);

        //Send email
        $result=$this->obj->email->send();
		if($result){
			return true;
		}else{
			return false;
	...
	function mail_template($to,$subject,$body){
		...
		$mailbody=$this->obj->load->view('page/email-template',$data,true);
	...
	function getTableData($table,$condition){
        $this->obj->db->select('*');
        $this->obj->db->from($table);
        $this->obj->db->where($condition);
    ...
    function converToTz($time="",$toTz='US/Eastern',$fromTz='US/Eastern')
    {   
